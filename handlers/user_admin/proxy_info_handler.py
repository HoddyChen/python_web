        ...
        if self.session['web_uid'] == None:
            ...
            if F.validate():
                ...
                if F.fx_type.data == "edit_proxy":
                    ...
                    page_main['th_num'] = 2
                    yield self.render("user/index_proxy_accountclass_list.html", page_main=page_main,
        ...
        if self.session['web_uid'] == None:
            ...
            if F.validate():#and F.cla.data == "SendError"
                ...
                if F.fx_type.data == "edit_proxy":
                    ...
                    flag = yield P.CheckProxyAccountClassId(self.session['web_uid'], F.gid.data)
                    if flag:
                        m_dist = yield P.set_change_proxy_account_class(self.session['web_uid'], F.gid.data, F.account.data)
                        if m_dist == True:
                            echo_dist['reponse_status'] = 5
                            echo_dist['echo'] = self.locale.translate("保存成功！")
                        else:
                    ...
                    # 读取分组列表
                    page_papa = {}
                    echo_dist['data'], echo_dist["recordsTotal"] = yield P.getProxyAccountListClassAll(
