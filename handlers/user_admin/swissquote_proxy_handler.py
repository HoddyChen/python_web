...
class SwissquoteProxyHandler(SessionHandler, BaseHandler):
        ...
        if self.session['swissquote_uid'] == None:
            ...
            if F.validate():
                ...
                if F.fx_type.data == "list_proxy_order":
                    ...
                    page_main['th_num'] = 3
                    yield self.render("user/swissquote_index_proxy_order_list.html", page_main=page_main, session=self.session)
                    return
                    ...
                    page_main['th_num'] = 4
                    yield self.render("user/swissquote_index_proxy_settlement_list.html", page_main=page_main, session=self.session)
                    return
                else:
        ...
        if self.session['swissquote_uid'] == None:
            ...
            if F.validate():#and F.cla.data == "SendError"
                ...
                if F.fx_type.data == "list_proposal":
                    echo_dist['data'] = yield P.getCsList(self.session['swissquote_uid'], F.start.data, F.length.data)
                elif F.fx_type.data == "list_proxy_order":
                    # 单个账户佣金列表
                    echo_dist['data'] = yield P.getProxyOrderList(self.session['swissquote_uid'], page_papa)
                elif F.fx_type.data == "list_proxy_count2":
                    # 单个账户返佣总金额
                    echo_dist['data'] = yield P.getProxyCount2(self.session['swissquote_uid'], page_papa)
                elif F.fx_type.data == "list_proxy_count":
                    # 返佣总金额
                    echo_dist['data'] = yield P.getProxyCount(self.session['swissquote_uid'], page_papa)
                elif F.fx_type.data == "list_proxy_order_count":
                    # 统计各账户的返佣金额
                    echo_dist['data'] = yield P.getProxyOrderCountList(self.session['swissquote_uid'], page_papa)
                elif F.fx_type.data == "list_settlement":
                    # 统计返佣结算金额列表
                    echo_dist['data'] = yield P.getProxySettlementList(self.session['swissquote_uid'], page_papa)
                elif F.fx_type.data == "list_settlement_count":
                    # 统计结算金额
                    echo_dist['data'] = yield P.getProxySettlementCount(self.session['swissquote_uid'], page_papa)
                else:
